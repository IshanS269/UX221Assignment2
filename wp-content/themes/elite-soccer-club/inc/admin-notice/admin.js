( function( $ ){
    $( document ).ready( function(){
      $( '.elite-soccer-club-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( elite_soccer_club_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'elite_soccer_club_dismissed_notice_handler',
              type: type,
            }
          } );
      } );
