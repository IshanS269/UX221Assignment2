...
<article id="post-<?php the_ID(); ?>" <?php post_class('blog-post'); ?>>
    <?php
        ...
        $elite_soccer_club_post_content = apply_filters('the_content', get_the_content());

        // Find and display the first image in the post content
        preg_match('/<img[^>]+src=[\'"]([^\'"]+)[\'"][^>]*>/i', $elite_soccer_club_post_content, $elite_soccer_club_matches);

        if (!empty($elite_soccer_club_matches[1])) {
            $elite_soccer_club_image_url = $elite_soccer_club_matches[1];
            echo '<div class="embedded-image"><img src="' . esc_url($elite_soccer_club_image_url) . '" alt="' . esc_attr(get_the_title()) . '"></div>';
        }
    ?>
