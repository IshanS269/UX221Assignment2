=== Sports Blog ===
Contributors: unitedtheme
Requires at least: 5.3
Tested up to: 6.7
Requires PHP: 5.6
Stable tag: 1.2.6
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html


== Description ==
Sports Blog is a clean modern WordPress Blog theme, carefully designed to help you create extraordinary blog sites very easily, featuring a large number of different styles and options. The theme offers a great user experience for almost any need and is extremely easily customizable. Every section and element is designed with simplicity and details in mind, and well-tailored to match your own taste. Sports Blog is also 100% responsive, that means it fits with all different kind of devices in different sizes.

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload Theme and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Frequently Asked Questions ==

= Does this theme support any plugins? =

Sports Blog includes support for Infinite Scroll in Jetpack.

== Copyright ==
Sports Blog WordPress Theme, Copyright 2018 YOUR UNITED THEME
Sports Blog is distributed under the terms of the GNU GPL

== Credits ==

Unless otherwise specified, all the theme files and scripts are licensed under GNU General Public License.

* Based on Underscores https://underscores.me/, (C) 2012-2017 Automattic, Inc., [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)
* normalize.css https://necolas.github.io/normalize.css/, (C) 2012-2016 Nicolas Gallagher and Jonathan Neal, [MIT](https://opensource.org/licenses/MIT)

* Google Fonts:

Source Sans Pro (https://fonts.google.com/specimen/Source+Sans+Pro)
Licensed under SIL Open Font License, 1.1 (http://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL_web)
Copyright Paul D. Hunt


Poiret One (https://fonts.google.com/specimen/Poiret+One)
Licensed under SIL Open Font License, 1.1 (http://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL_web)
Copyright Denis Masharov

Webfont Loader:
Copyright: Copyright (c) 2020 WPTT
URL:  https://github.com/WPTT/webfont-loader
License: MIT License

* Slick, Copyright (c) Ken Wheeler, Licensed under MIT,  http://github.com/kenwheeler/slick/
* normalize.css, Copyright 2012-2016 Nicolas Gallagher and Jonathan Neal Licensed under MIT, https://necolas.github.io/normalize.css/
* Theia Sticky Sidebar v1.7.0  MIT License (MIT) https://github.com/WeCodePixels/theia-sticky-sidebar
* jquery-match-height, Copyright liabru Licensed under the MIT license, http://brm.io/jquery-match-height/

Image Used from stocksnap.io and is released under CC0 Creative Commons:

https://stocksnap.io/photo/people-boy-20JRE92170
https://stocksnap.io/photo/boxing-ring-PPBG50NAWP
https://stocksnap.io/photo/sports-ball-HWDNFMZHRP
https://stocksnap.io/photo/golf-golfing-0DJOIMORO0
https://stocksnap.io/photo/man-kettlebell-RANNCGTVFH

== Changelog ==

1.2.6 - Feb 08 2025
* New - TikTok Social Icon
* Update - Minor design issue

1.2.5 - Oct 04 2024
* Update - WordPress 6.6 compatibility

1.2.4 - July 05 2024
* Update - WordPress 6.5 compatibility
* Update - Minor design issue

1.2.3 - March 10 2024
* Update - Admin Dashboard

1.2.2 - Jan 12 2024
* Update - 404 Error Page
* New - Fixed Floating navigation

1.2.1 - Nov 09 2023
* Update - 6.3 compatibility
* Update - Discord Icon

1.2.0 - Oct 02 2023
* Update - wp_add_inline_style
* Update - Various Small fixes

1.1.8 - Aug 03 2023
* Update - Header meta viewport
* Update - Twitter Logo
* Update - Magnific Popup delegate

1.1.7 - June 02 2023
* Update - Various Small fixes

1.1.6 - May 12 2023
* Update - 6.2 compatibility
* Update - Various Small fixes

1.1.5 - March 22 2023
* Update - RTL support
* Fixed  - Various small fixes.

1.1.4 - Dec 26 2022
* Fixed about page issue
* Updated new CSS and other quick fixing

1.1.3 - Feb 22 2022
* Add about page
* New language file generated

1.1.2 - Nov 16 2021
* Add custom Cursor

1.1.1 - Sep 13 2021
* Added gutenberg block support
* Added responsive-embeds support
* Added wp-block-styles support
* Fixed typo issue on script.js
* Google font loader added.

1.1.0 - April 11 2021
* Icons upgrades to svg icons.

1.0.9 - February 10 2021
* Updated fixing skip link.
* Preloader issue fixed on mozilla browser

1.0.8 - Nov 04 2020
* updated fixing skip link
* updated fixing some css glitch

1.0.7 - Aug 04 2020
* updated fixing  content issue

1.0.6 - Aug 02 2020
* updated fixing some CSS and content issue

1.0.5 - Mar 18 2020
* updated fixing keyboard navigation for pop up search

1.0.4 - Nov 01 2019
* updated with language file compatibility

1.0.3 - Aug 19 2019
* Added Theia Sticky Sidebar
* Added the wp_body_open action.
* Added skip links TRT requirement.
* Managed RTL compatibility and added the rtl-language-support tag.

1.0.2 - Sep 12 2018
* Updated new change.

1.0.1 - Aug 30 2018
* Updated theme tags.

1.0.0 - July 14 2018
* Initial release

